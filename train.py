...
def train():
    ...
    trainSet = PSGdataset(DATASET, 'config/sleepedf_train.txt')
    print("----------Training----------")
    trainSet.showInfo()
    ...
    validSet = PSGdataset(DATASET, 'config/sleepedf_valid.txt')
    print("----------Validation----------")
    validSet.showInfo()
    validSampler = SleepSampler(validSet, groupSize=1)
    validLoader = DataLoader(validSet,
                            sampler=validSampler,
                            batch_size=BATCHSIZE,
                            drop_last=False)
    
    testSet = PSGdataset(DATASET, 'config/sleepedf_test.txt')
    print("----------Testing----------")
    testSet.showInfo()
    testSampler = SleepSampler(testSet, groupSize=1)
